...
1. **Frontend:** Collects property features, validates inputs, and queries the backend.
2. **Backend:** Exposes endpoints, validates types using Pydantic, and handles inference.
3. **Machine Learning Pipeline:** Preprocesses unstructured inputs (scales numeric features, maps categorical columns, imputes missing values) and outputs the predicted valuation using a trained Random Forest Regressor.
...
The system is trained on the [House Price dataset by Juhi Bhojani on Kaggle](https://www.kaggle.com/datasets/juhibhojani/house-price). It contains approximately 187,000 real estate listings in India.

### To Download:
1. Download the dataset directly from Kaggle.
2. Unzip the file and place `house_prices.csv` into your project structure at:
...
*(Place screenshots of your running React application and FastAPI Swagger documentation here in your repository)*

| Frontend Form | Prediction Result |
| :---: | :---: |
| ![Form Page](https://via.placeholder.com/300x200?text=Frontend+Form+Screenshot) | ![Result Page](https://via.placeholder.com/300x200?text=Prediction+Result+Screenshot) |
```